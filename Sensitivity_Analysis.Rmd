...
```{r, include=F}
dist_matrices = list(IBS=as.dist(IBS_WG),
                     IBD=as.dist(IBD_WG),
                     IBD_neglog=as.dist(IBD_neglog2_WG))
link_methods = c('complete')
dend_list = list()
i = 1
...
```{r, include=F}
dist_matrices = list(logIBD_flank = as.dist(IBD_neglog2_WG))
